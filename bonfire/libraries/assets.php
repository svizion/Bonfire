...
class Assets
{
	...
	/**
	 * An array of inline script variables to be placed before the scripts are added.
	 *
	 * @access protected
	 *
	 * @var array
	 */
	protected static $vars_js_scripts	= array();
		...
		if (!$combine)
		{
			foreach ($styles as $s)
			{
				...
				$attr = array(
					'rel'	=> 'stylesheet',
					'type'	=> 'text/css',
					'href'	=> is_array($s) ? $s['file'].'?_dt='.time() : $s.'?_dt='.time(),
	...
	/**
	 * Returns all PHP vars that were turned to json the vampire objects.
	 * @todo : note see add_js_var
	 * @return string
	 */
	public function vars_js()
	{
		...
		$scripts = self::$vars_js_scripts;

		if ( FALSE === count($scripts) )
		{
			return NULL;
		}
	...
	/**
	 * Converts PHP array into JSON object and adds it to array of js vars.
	 *
	 * @param string $name_space  Namespace of the JSON object to create or add to.
	 * @param string $object_name Object name
	 * @param array  $data        Array of key => value vars to be added to javascript above, script loading.
	 */
	public function add_js_var( $name_space, $object_name, $data )
	{
		if ( is_array($data) && isset($data['print_after']) )
		{
			$after = $data['print_after'];
			unset($data['print_after']);
		}
		...
		$script  = "var {$name_space} = {$name_space} || {};" . PHP_EOL;
		$script .= "	{$name_space}.{$object_name} = " . json_encode($data) . ';';
		$script .= PHP_EOL;

		if ( isset($after) && ! empty($after) )
			$script .= "\n$after;";

		self::$vars_js_scripts[] = $script;
		return $script;
	}
